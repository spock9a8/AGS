...
public class GifSequenceWriter {
	...
	public static void makeGif(String[] args) throws IOException {
		// get animation length in milliseconds
		int millis = Integer.parseInt(args[args.length - 2]);

		// grab the output image type from the first image in the sequence
		BufferedImage firstImage = ImageIO.read(new File(args[1]));
		...
		GifSequenceWriter writer = new GifSequenceWriter(output, firstImage.getType(), millis, true);
	...
	public static void decompileGif(String[] args) throws IOException {
		ImageReader reader;
		ImageInputStream ciis;
		BufferedImage master, in;
		int direction = Integer.parseInt(args[args.length - 2]);
		if (direction != 0 && direction != 1) {
			printUsage();
			return;
		}
		reader = (ImageReader) ImageIO.getImageReadersByFormatName("gif").next();
		ciis = ImageIO.createImageInputStream(new File(args[1]));
		in = ImageIO.read(new File(args[1]));
		reader.setInput(ciis, false);
		int noi = reader.getNumImages(true);

		master = (direction == 0) ? new BufferedImage((in.getWidth() * noi), in.getHeight(), BufferedImage.TYPE_INT_ARGB) : new BufferedImage((in.getWidth()), in.getHeight() * noi, BufferedImage.TYPE_INT_ARGB);
		...
		g.dispose();
		ImageIO.write(master, "PNG", new File(args[args.length - 1]));
	}

	public static void printUsage() {
		System.out.println("Usage: \n      java -jar agc.jar [compile | decompile] [List of Images | Gif Image] [MS per frame | Direction(H:0, V:1)] [outputFile.gif | outputFile.png]");
